...
import usePplxTtsRequest from "@/plugins/_thread/message-tts/hooks/usePplxTtsRequest";
import type manifest from "@/plugins/_thread/message-tts/index.manifest";
import { PplxTtsPlayerCoordinator } from "@/plugins/_thread/message-tts/player/coordinator";
import type { TtsVoice } from "@/plugins/_thread/message-tts/types";
import { TTS_VOICES } from "@/plugins/_thread/message-tts/types";
...
export function ThreadMessageTtsButton() {
  ...
  const [voice, setVoice] = useLocalStorage<TtsVoice>(
    "cplx.plugin:thread:messageTts:voice" satisfies `cplx.plugin:${typeof manifest.meta.id}:${string}`,
    "Mike",
  );
