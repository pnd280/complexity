...
export class MarkmapRendererServiceImpl {
  ...
  render = throttle(
    async ({
      ...
      try {
        ...
        const transformer = (window as any).markmapTransformer as Transformer;
        ...
        if (!document.body.contains(this.currentMarkmapSvgNode)) {
          ...
          const { Markmap } = (window as any).markmapView as typeof markmapView;
  ...
  private getInteractiveHtml({ content }: { content: string }) {
    const fillTemplate = ((window as any).markmapRender as typeof markmapRender)
      .fillTemplate;
