...
export const getRawItems = (): CommandItemProps[] => {
  const keybindings =
    PluginsSettingSnapshotsService.getPluginSnapshot("commandMenu").keybindings;
  ...
  return [
    {
      ...
      icon: PplxThread,
      keybinding: normalizeCommandMenuKeybinding(keybindings.threadsSearch),
      keywords: ["search"],
      onSelect: () => {
    ...
    },
    {
      ...
      group: t("plugin-command-menu.groups.search"),
      icon: PplxSpace,
      keybinding: normalizeCommandMenuKeybinding(keybindings.spacesSearch),
      keywords: ["search"],
      onSelect: () => {
        commandMenuStore.getState().pagesStack.push({
          pageId: "spaces",
