...
function findActiveQueryBoxTextarea({
  type,
}: {
  type?: 'main' | 'follow-up';
  ...
  if (type === 'main') {
    const $main = $parents.find('textarea[placeholder="Ask anything..."]:last');

    const $collection = $parents.find(
      'textarea[placeholder="New Thread"]:last'
    );

    return ($main.length ? $main : $collection) as JQuery<HTMLTextAreaElement>;
  }
...
function findActiveQueryBox({ type }: { type?: 'main' | 'follow-up' }) {
  return findActiveQueryBoxTextarea({
    type,
  }).parents('.grow.block');
}
...
function getWordOnCaret(element: HTMLTextAreaElement) {
...
function findMostVisibleElementIndex(elements: Element[]) {
...
function getMessageBlocks() {
  ...
  $messagesContainer.children().each((_, messageBlock) => {
    ...
    $messageBlock.attr('id', 'message-block');

    const { $query, $answer, $answerHeading } =
      parseMessageBlock($messageBlock);

    $messageBlock.find('.col-span-8:last').attr('id', 'text-col');
    $messageBlock.find('.col-span-4:last').attr('id', 'visual-col');
